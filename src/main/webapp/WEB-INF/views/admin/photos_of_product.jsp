	            ...
	            <div class="card">
	              <div class="card-body">
	                <table id="colorList" class="table table-bordered table-striped">
	                  ...
	                  <tbody>
	                  <c:forEach var="c" items="${products.photos }">
		                  <tr>
		                    <td>${c.id }</td>
		                    <td><img alt="${c.products.name}" src="${pageContext.request.contextPath}/uploads/images/${c.name}" width="100" height="100"/></td>
		                    <td>${c.status ? 'Show' : 'Hide' }</td>
		                    <td>${c.main ? 'Main' : '' }</td>
		                    <td>
		                    	<a href="${pageContext.request.contextPath}/home/admin/photo/edit/products/${c.id}">Edit</a> 
		                    	| 
		                    	<a href="${pageContext.request.contextPath}/home/admin/photo/delete/products/${c.id}/${c.products.id}" onclick="return confirm('Are you sure?')">Delete</a>
		                    </td>
		                  </tr>
	                  </c:forEach>
	                  </tbody>
